...
The most suitable method for studying  single particle excitation spectra such as ionization energies and electron affinities of extended systems is the Green's function. The Green's function relies on the calculation of the self-energy operator which is non-local and energy dependent.  The self-energy is best approximated by the so called quasiparticle $GW$ approximation. $GW$ stands for the single-particle Green's function ($G$) and the dynamically screened Coulomb interaction ($W$). In practice, the exchange-correlation potential is replaced by the self-energy $\Sigma$

\begin{equation}
	V_{xc}(\va*{r}) \phi_i(\va*{r}) \rightarrow \int \Sigma(\va*{r},\va*{r}',\epsilon_i) \phi_i(\va*{r})\, \dd{r'} 
\end{equation}
 $\Sigma$ gives the finite discontinuity of $\Delta_{xc}$ in \eqref{eq:xc_discon}, thus yields the correct quasiparticle band gap. 
